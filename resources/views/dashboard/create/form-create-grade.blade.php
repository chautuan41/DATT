...
<div class="app-title">
<div>
    <h1><i class="fa fa-edit"></i>Thêm Lớp Học</h1>
</div>
...
<div class="col-md-12">
    ...
    <div class="col-lg-6">
        ...
        @if($errors)
            ...
            @endforeach
        @endif
        <form method="POST" acction="{{route('create-grade')}}">
            @csrf
            <div class="form-group">
            <label for="exampleInputEmail1">Lớp</label>
            <input class="form-control" name="grade_name" type="text" value="" required> 
            @if($errors->has('grade_name'))
            <span>{{$errors->first('grade_name')}}</span>
            @endif
            </div>
